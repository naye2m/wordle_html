...
function win() {
    localStorage.win ??= 0;
    localStorage.win++;
    alert("you winn the game. Restarting new game.You winn this game "+ localStorage.win);
    setTimeout(window.location.reload(), 2000);
}
...
function print_word(guess, wordsize, status) {
    ...
    const colorList = ["r", "y", "g"];
    // TODO #6
    let str = "";
    for (let i = 0; i < wordsize; i++) {
        // str += `${colorList[status[i]]}${guess[i]}`;
        str += `<td class="${colorList[status[i]]}">${guess[i]}</td>`;
    }
    str = `<tr>${str}</tr>`;
    console.log(str);
    return str;
